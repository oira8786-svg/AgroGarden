...
@Composable fun CropScreen(db:AppDb){val list by db.crops().all().collectAsState(emptyList()); val scope=rememberCoroutineScope(); var name by remember{mutableStateOf("")}
 Column{Row{OutlinedTextField(name,{name=it},modifier=Modifier.weight(1f),label={Text("Культура")}),Modifier.weight(1f));Button({if(name.isNotBlank())scope.launch{db.crops().add(Crop(name=name,area=0.0,sowDate="",harvestDate="",status="Запланировано",notes=""))};name=""},Modifier.padding(start=8.dp)){Text("➕")}}
